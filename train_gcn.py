...
from src.evaluate import ndcg_at_k, per_user_topk_report, recall_at_k
from src.graph_builder import (
    build_id_mappings,
    build_multirelation_graph,
    build_node_features,
)
from src.models import GCNRecommender
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a classic GCN recommender.")
    parser.add_argument("--epochs", type=int, default=5)
    parser.add_argument("--embedding-dim", type=int, default=16)
    parser.add_argument(
        "--feature-only",
        action="store_true",
        help="Disable learned node ID embeddings and use only fixed node features.",
    )
    ...
    parser.add_argument(
        "--results-path",
        type=Path,
        default=Path("outputs/gcn_results.csv"),
    )
    parser.add_argument(
        "--user-report-path",
        type=Path,
        default=Path("outputs/gcn_test_user_report.csv"),
    )
...
def main() -> None:
    ...
    effective_embedding_dim = 0 if args.feature_only else args.embedding_dim
    ...
    best_train_metrics = evaluate_split(
        model,
        adjacency,
        node_features,
        train,
    ...
    best_test_recommendations = build_recommendations(
        model,
        adjacency,
        node_features,
        test,
    ...
    test_user_report = per_user_topk_report(test, best_test_recommendations, k=args.top_k)
    args.user_report_path.parent.mkdir(parents=True, exist_ok=True)
    test_user_report.to_csv(args.user_report_path, index=False)
    ...
    print(f"GCN Train Recall@{args.top_k}: {best_train_metrics['recall_at_k']:.4f}")
    print(f"GCN Train NDCG@{args.top_k}: {best_train_metrics['ndcg_at_k']:.4f}")
    print(f"GCN Val Recall@{args.top_k}: {best_val_metrics['recall_at_k']:.4f}")
    print(f"GCN Val NDCG@{args.top_k}: {best_val_metrics['ndcg_at_k']:.4f}")
    print(f"GCN Test Recall@{args.top_k}: {best_test_metrics['recall_at_k']:.4f}")
    print(f"GCN Test NDCG@{args.top_k}: {best_test_metrics['ndcg_at_k']:.4f}")
    print(f"Popularity Train Recall@{args.top_k}: {best_train_metrics['popularity_recall_at_k']:.4f}")
    print(f"Popularity Train NDCG@{args.top_k}: {best_train_metrics['popularity_ndcg_at_k']:.4f}")
    ...
    print(f"GCN Train Recall@{args.top_k}: {final_train_metrics['recall_at_k']:.4f}")
    print(f"GCN Train NDCG@{args.top_k}: {final_train_metrics['ndcg_at_k']:.4f}")
    print(f"GCN Val Recall@{args.top_k}: {final_val_metrics['recall_at_k']:.4f}")
    print(f"GCN Val NDCG@{args.top_k}: {final_val_metrics['ndcg_at_k']:.4f}")
    print(f"GCN Test Recall@{args.top_k}: {final_test_metrics['recall_at_k']:.4f}")
    print(f"GCN Test NDCG@{args.top_k}: {final_test_metrics['ndcg_at_k']:.4f}")
    print(f"Popularity Train Recall@{args.top_k}: {final_train_metrics['popularity_recall_at_k']:.4f}")
    print(f"Popularity Train NDCG@{args.top_k}: {final_train_metrics['popularity_ndcg_at_k']:.4f}")
    ...
    append_results(
        args.results_path,
        {
            ...
            "best_train_recall_at_k": best_train_metrics["recall_at_k"],
            "best_train_ndcg_at_k": best_train_metrics["ndcg_at_k"],
            ...
            "final_train_recall_at_k": final_train_metrics["recall_at_k"],
            "final_train_ndcg_at_k": final_train_metrics["ndcg_at_k"],
            ...
            "popularity_train_recall_at_k": final_train_metrics["popularity_recall_at_k"],
            "popularity_train_ndcg_at_k": final_train_metrics["popularity_ndcg_at_k"],
    ...
    print(f"Saved GCN experiment results to {args.results_path.resolve()}")
    print(f"Saved per-user test report to {args.user_report_path.resolve()}")
